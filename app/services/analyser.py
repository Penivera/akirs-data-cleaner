...
def generate_markdown_report(state, rows: List[Tuple[Any, ...]], header_row_idx: int) -> str:
    ...
    for col in keep_columns:
        if col in header_map and col not in [identity_col, metric_col, currency_col, flow_type_col]:
    ...
    groups = defaultdict(lambda: {"metric_sum": 0.0, "inflow_sum": 0.0, "outflow_sum": 0.0, "count": 0, "metadata": {}})
    ...
    for i, row in enumerate(rows[header_row_idx+1:], start=1):
        ...
        if flow_idx is not None and flow_idx < len(row):
            f_val = str(row[flow_idx]).strip().upper() if row[flow_idx] is not None else ""
        ...
        target_group["scaled_metric_sum"] = target_group.get("scaled_metric_sum", 0.0) + (m_val * get_fx_rate(c_val))
    ...
    sorted_groups = sorted(groups.items(), key=lambda x: x[1].get("scaled_metric_sum", x[1]["metric_sum"]), reverse=True)
    ...
    for c_val, t_sum in total_metric_sums.items():
        ...
        lines.append(f"   - Total Inflows: {format_currency(total_inflows[c_val], c_val)}")
        lines.append(f"   - Total Outflows: {format_currency(total_outflows[c_val], c_val)}")
    ...
    for idx, (group_key, data) in enumerate(top_n, 1):
        ...
        lines.append(f"   Inflows: {format_currency(data['inflow_sum'], c_val)} | Outflows: {format_currency(data['outflow_sum'], c_val)}")
    ...
    for c_val, t_sum in total_metric_sums.items():
        ...
        lines.append(f"- Total '{metric_col}' of Top {limit}: {format_currency(c_top_sum, c_val)}")
        if c_top:
            lines.append(f"- Highest Single '{metric_col}': {c_top[0][0][0]} ({format_currency(c_top[0][1]['metric_sum'], c_val)})")
            lines.append(f"- Average '{metric_col}' per Top Account: {format_currency(c_top_sum / len(c_top), c_val)}")
        lines.append("")
            
    lines.append(f"- Most Active Record (Overall): {most_active_k} ({most_active_v} transactions)")
    lines.append("")
