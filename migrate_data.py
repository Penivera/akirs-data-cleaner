...
source_file = 'OCT  AKWA IBOM NEW CUSTOMERS Q3 2025.xlsx'
target_file = 'OCT  AKWA IBOM NEW CUSTOMERS Q3 2025.csv'
...
def migrate_data():
    ...
    with open(target_file, mode='w', newline='', encoding='utf-8') as f:
        ...
        for row in rows[2:]:
            ...
            taxpayer_id = get_val(row, 'TIN')
            account_name = get_val(row, 'ACCOUNT NAME')
            nuban = get_val(row, 'NUBAN')
            bvn = get_val(row, 'BVN')
            phone = get_val(row, 'PHONE NO 1')
            address = 'N/A'  # Hardcoded per user request
